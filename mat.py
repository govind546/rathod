...
dat=pd.read_csv('C:\\Users\\acer33\\Documents\\mt.csv') # reading csv file to a local data frame 
...
mc=dat.iloc[:,'MPG.city'].values # storing the values in varible
hs=dat.iloc[:,'Horsepower'].values# storing the values in varible
...
ser.reset_index(inplace=True)# afcedding a new index for dataframe
